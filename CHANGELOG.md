...
### AC-2 + AC-7 Fixed · ai-gov rescan command · Governance File Structure HTML

All 8 production-ready acceptance criteria now pass. Both blocking ACs are resolved by the framework self-governing via `ai-gov init`, and a new `ai-gov rescan` command gives teams a targeted, non-destructive way to refresh scanner detection when tooling changes.

### Added

- **`src/commands/rescan.ts`** — new `ai-gov rescan` command. Re-runs the project scanner and updates the `ScannerSnapshot` in `governance-state.json` without touching any steering files, hooks, or specs. Use when ORM upgraded, dependencies changed, architecture refactored. Errors clearly if `governance-state.json` doesn't exist (run `ai-gov init` first). Handles corrupted state file with a clear error message.
- **`docs/governance_file_structure.html`** — new standalone interactive HTML guide. Shows every file generated by `ai-gov init` (steering, commands, hooks by trigger type, git hooks, knowledge hub). Dark IDE-style layout with fixed sidebar, color-coded file types, detail drawer (click any file for trigger, purpose, contents), full directory tree for both Claude Code and Kiro. Offline-capable — no CDN except Google Fonts.
- **`.claude/governance-state.json`** — framework now self-governs. `ai-gov init` run on the framework's own source directory produces this file with a fully populated `scannerSnapshot` (15 fields with confidence metadata).

### Changed

- **`src/cli.ts`** — registers new `rescan` command after `migrate-state`.
- **CI generators** (`github.ts`, `gitlab.ts`, `bitbucket.ts`) — install script updated: `ai-gov@20.3.0` → `ai-gov@20.5.0`.

### Fixed

- **AC-2** — `governance-state.json` now present. Schema v1 · ai-gov (react) · 0 audit runs.
- **AC-7** — `scannerSnapshot` populated with all 15 confidence-wrapped fields. Doctor verdict: PASSING.

### Doctor output after this release

```
✓ AC-1 — Kiro parity for assess + backlog
✓ AC-2 — Shared governance-state.json
✓ AC-3 — /assess runs with zero human-input gates
✓ AC-4 — /backlog runs with zero human-input gates
✓ AC-5 — /doctor production-ready emits mechanical verdict
✓ AC-6 — Generator tests at parity
✓ AC-7 — Scanner confidence wrapper on 15 fields + --rescan
✓ AC-8 — Completion contracts on every command

PASSING — v20 acceptance criteria satisfied.
```

### Version alignment

- `package.json` 20.4.0 → 20.5.0
- `src/constants.ts` VERSION + HOOK_VERSION → 20.5.0
- CI install scripts → `ai-gov@20.5.0`
- `tests/knowledge-hub.test.ts` version assertion updated

### Migration

No migration required. `ai-gov rescan` is additive — projects without `governance-state.json` get a clear error directing them to `ai-gov init`. The HTML guide is a local documentation file only, not part of generated governance output.
