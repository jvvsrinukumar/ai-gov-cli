...
describe('CLI --agent routing', () => {
    ...
    test('kiro hooks are .kiro.hook files, not bash scripts', () => {
        const config = makeConfig('kiro', 'nodejs', {}, { projectDir: tmpDir });
        runGovernance(config);
        const hookFile = join(tmpDir, '.kiro', 'hooks', 'block-dangerous-commands.kiro.hook');
    ...
    test('kiro init with specFirstEnabled generates spec-first-gate hook', () => {
        ...
        runGovernance(config);
        expect(existsSync(join(tmpDir, '.kiro', 'hooks', 'spec-first-gate.kiro.hook'))).toBe(true);
    });
