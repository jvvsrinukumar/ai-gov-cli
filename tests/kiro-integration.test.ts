...
describe('Kiro init integration', () => {
    ...
    test('fresh kiro init on React project: all expected .kiro/ files exist', () => {
        ...
        expect(existsSync(join(tmpDir, '.kiro', 'hooks', 'block-dangerous-commands.json'))).toBe(true);
        expect(existsSync(join(tmpDir, '.kiro', 'hooks', 'protect-files.json'))).toBe(true);
        expect(existsSync(join(tmpDir, '.kiro', 'hooks', 'check-secrets.json'))).toBe(true);
    });
    ...
    test('fresh kiro init on Node.js project: hooks reference .js extensions', () => {
        ...
        runGovernance(config);
        const secrets = readFileSync(join(tmpDir, '.kiro', 'hooks', 'check-secrets.json'), 'utf-8');
...
describe('Kiro workspace integration', () => {
    ...
    test('workspace kiro init creates per-project .kiro/hooks/', () => {
        runWorkspaceInit({ dir: wsDir, dryRun: false, overwrite: true, agent: 'kiro' });
        expect(existsSync(join(wsDir, 'frontend', 'web', '.kiro', 'hooks', 'block-dangerous-commands.json'))).toBe(true);
        expect(existsSync(join(wsDir, 'backend', 'api', '.kiro', 'hooks', 'block-dangerous-commands.json'))).toBe(true);
    });
...
describe('Kiro upgrade integration', () => {
    ...
    test('kiro upgrade regenerates hooks', () => {
        ...
        const hookPath = join(tmpDir, '.kiro', 'hooks', 'block-dangerous-commands.json');
