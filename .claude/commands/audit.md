...
> **Project:** ai-gov
> **Stack:** Node.js
> **Init detected:** state: N/A · tests: jest · linter: eslint *(baked at init time — Step 5 will re-discover actual state)*
> **Init structural note:** No legacy zones detected at init — code scan will confirm
> **Hook version:** v15.2.0
> **High-risk files:** config.ts
...
```
protect-files.sh, block-dangerous-commands.sh, check-secrets.sh, session-continuity.sh, format-code.sh, analyze-code.sh, check-feature-readme.sh, check-consistency.sh, check-file-size.sh, post-task-checklist.sh
```
Conditional: check-spec-exists.sh — conditional: registered in settings.json only when `specs/` has feature directories beyond `_template/`. Intentionally absent on a fresh project.

For each hook: file exists? ✓ · version matches `15.2.0`? ✓ or STALE [found: X.Y.Z] · file non-empty? ✓
...
For each significant directory under `src/`, read 15-25 files and answer:
...
```
src/routes/      [N] files — patterns: [describe]
src/controllers/ [N] files — patterns: [describe]
src/services/    [N] files — patterns: [describe]
...
List every feature-sized directory in `src/`. For each:
...
- Files with `_old`, `_backup`, `_deprecated`, `_unused`, `Old`, `Backup` in filename
...
- Empty `src/` subdirectories (no .ts files inside)
...
```
PROJECT TRUTH AUDIT — ai-gov
Stack: Node.js | Hook version: v15.2.0
Date: <today>
...
━━━ VERDICT ━━━
  ALIGNED
    ...
    [N] hooks at v15.2.0. Development can start.
  OR
...
## How specs work in ai-gov

Code lives in `src/`. Each feature follows the layer flow: `Route → Model`

Before writing code, spec:
- **Route layer** — inputs, outputs, events the user triggers
- **Model layer** — business rules, validation, what can fail
- **Model layer** — data shape, queries, external calls

**Task types:** New Feature · Edit Feature · Bug Fix · Refactor · Hotfix

**To activate spec-first workflow:**
```bash
cp -r specs/_template/ specs/user-profile/
