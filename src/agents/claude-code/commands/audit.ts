...
- Are HTTP interceptors registered? (auth token injection, error handling, loading state?) List them from \`provideHttpClient(withInterceptors([...]))\` or \`HTTP_INTERCEPTORS\` providers.
...
- FastAPI: How are services injected? Through \`Depends()\`, instantiated at module level, or created inside each function? Are database sessions passed via \`Depends(get_db)\` or opened inside endpoint functions?
- Django: Are services plain classes instantiated in views, or does the project use signals/middleware for cross-cutting concerns?
...
- Are return types annotated on endpoint functions?

**Background tasks**
- Is Celery, RQ, ARQ, or FastAPI \`BackgroundTasks\` used?
- Where do task definitions live? (\`tasks/\`, \`workers/\`, co-located with routers?)

**Migrations**
- Is Alembic present? Is \`alembic.ini\` at the root? Are migration files in \`alembic/versions/\`?
- If Django: \`migrations/\` directories in each app?

**Data flow**
- Trace one real endpoint: from route decorator to database call. List every function it passes through.

**Async patterns**
- Are endpoints \`async def\` or sync \`def\`? Are database calls awaited consistently?
- Are tests using \`pytest-asyncio\` for async endpoints? (sync tests on async endpoints pass silently — note if mixed)

**Naming and conventions**
- What file naming convention? What router/view prefix/tag pattern?

**Per directory — record as facts:**
*(FastAPI/Flask: use structure below. Django: substitute \`apps/<name>/views.py\`, \`apps/<name>/models.py\`, \`apps/<name>/serializers.py\` etc.)*
\`\`\`
...
- Are StateFlows collected with \`collectAsState()\` in Composables or \`lifecycleScope.launch\` in Fragments?
...
**Naming and conventions**
- What naming pattern for ViewModels, UseCases, Repositories?
- What package structure per feature?
...
**Naming and conventions**
- What naming pattern for Views, ViewModels, Services?
- What directory structure per feature?
...
- Add to coding-standards.md: "Tests not yet set up — run \`${testCmd}\` once configured"
- Score: 0/100
...
- FastAPI: are they using \`TestClient\` or \`AsyncClient\` for endpoint tests?
...
- Do they use \`StandardTestDispatcher\` or \`UnconfinedTestDispatcher\` (not the deprecated \`TestCoroutineDispatcher\`)?
