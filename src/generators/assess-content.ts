/**
 * Shared assess prompt body — consumed by Claude Code (/assess) and Kiro
 * (workflow-assess). v20 plan §5 risk register: shared content prevents the
 * Kiro/Claude drift that re-opens the parity gap over time.
 */
...
export interface AssessContentParams {
    config: GovernanceConfig;
    /** User-facing command name in headings and prose. */
    commandName: string;
    /** Agent label for the "Assessed by:" line. */
    agentLabel: string;
    /** Knowledge preamble — Command-style for Claude, Hook-style for Kiro. */
    knowledgePreamble: string;
    /** Path the rubric reads for aged-action signals, agent-specific. */
    developerActionsPath: string;
}

export function generateAssessContent(p: AssessContentParams): string {
    const { config: c, commandName, agentLabel, knowledgePreamble, developerActionsPath } = p;
    ...
    return `# ${commandName} — Refactor vs Rewrite Decision Framework
...
> 10. **No human-input gates.** Every score in Doc 02 (including Business Pressure) is derived from observable evidence per the rubric below. Every inferred value carries an \`assumptions\` entry with evidence and confidence; the developer can override post-generation. The pipeline never waits for input.
> 11. **Completion contract — emit on the very last line of the run, exactly:**
>     \`ASSESS_COMPLETE: docs-written=<N>/11 recommendation=<Rewrite|Refactor|Strangler|Leave-It> confidence=<high|medium|low>\`
>     The contract is the structural signal that the run finished. The runner greps for it; an absent contract means the run did not complete and the verdict is incomplete regardless of any prose summary above it.
...
**Date:** <today>
**Stack:** ${stackDisplay}
**Assessed by:** ${agentLabel} via ${commandName}
...
| Business pressure | [1-4] | [inferred from rubric — see **Business Pressure Inference** below] |
...
**Business Pressure Inference** (no human gate — derive from observable evidence):

Score each signal. Sum to get the composite score, then bucket.

| Signal | How to observe | Weight |
|---|---|:---:|
| Recent bug commits on core modules (last 90 days) | \`git log --grep="fix\\|bug" --since="90 days ago" -- ${sourceDir}\` | +1 per ≥5 commits |
| Aged developer-actions (\`[ ] OPEN\` items older than 60 days) | Read \`${developerActionsPath}\` | +1 per ≥3 aged items |
| Contributor churn — active (6mo) / total all-time | Step 4 git archaeology | <25% → +1 |
| EOL or 2+ major-behind dependencies in critical path | Step 2 result | ≥1 → +1; ≥3 → +2 |
| Revert / hotfix density on core (last 90 days) | \`git log --grep="revert\\|hotfix" --since="90 days ago"\` | ≥3 → +1 |
| Core stable — no commits to core dirs in last 12 months | Step 4 result | true → −2 |

**Composite → bucket:**
- ≤ 0 → score **1** (No complaints)
- 1–2 → score **2** (Features blocked occasionally)
- 3–4 → score **3** (Slowing delivery noticeably)
- ≥ 5 → score **4** (Platform-level pressure — change required)

**CONFIDENCE by signals fired:** ≥3 signals = High · 2 = Medium · ≤1 = Low. The pipeline **never blocks**; Low-confidence outputs continue but are flagged \`reviewRequired: true\`.

**Emit an ASSUMPTIONS block alongside the recommendation** (machine-readable — Phase D wiring will append it to \`governance-state.json\` \`assumptions[]\`):

\`\`\`yaml
- field: assessment.businessPressure
  inferredValue: <1|2|3|4>
  evidence:
    - "<signal name>: <observation, e.g. '12 fix/bug commits in last 90 days on src/payment/'>"
    - "<signal name>: <observation>"
  confidence: <high|medium|low>
  reviewRequired: <true if confidence=low, else false>
  timestamp: <ISO 8601>
\`\`\`
...
CONFIDENCE:        [High / Medium / Low]
BUSINESS PRESSURE: <score 1-4> (composite=<sum>, signals fired=<count>)
REVIEW REQUIRED:   <true if confidence=Low, else false>
\`\`\`
...
━━━ ASSESSMENT COMPLETE — ${project.appName} ━━━
  ...
  Next steps:
    1. Review 02_decision.md — verify the inferred Business Pressure signals and confidence
       (override only if the evidence is wrong; the rubric does not require human input)
...
Substitute the actual counts and values. No other text after it. The runner uses this line to verify the assessment finished — without it, the run is treated as incomplete regardless of prose above.
`;
}
